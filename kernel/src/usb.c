...
void UhciInit() {
    uint32_t device_address = pci_find_next_device(UHCI_CONTROLLER_ADDRESS, UHCI_CONTROLLER_MASK, 0);
    uint16_t io_address = pci_read_config_u32(device_address, BAR4_OFFSET) & ~0x3; //Add better function in pci
    ...
    while (1){
        for (int i = 0; i < 17; i++){
            uint8_t curr_register = port_in_u8(io_address+i); 
            put_hex_32(curr_register, 30, 16+i); 
            put_hex_32(io_address+i, 10, 16+i); 
